...
==================
Finds every new X/Twitter mention of Fractile (the AI inference-hardware company) and posts it to Slack.
...
Routing:
  confident Fractile          -> MAIN channel (clean feed of real mentions)
  confident NOT Fractile      -> dropped (we looked, it isn't us)
...
STRONG_TEXT_RE = re.compile(r"fractile\.ai", re.I)
ETCHED_WORD_RE = re.compile(r"(?<![a-z])fractile(?![a-z])", re.I)
SOHU_WORD_RE   = re.compile(r"(?<![a-z])fractile(?:['’]s)?(?![a-z])", re.I)
FRACTILE_STATS_RE = re.compile(r"\b(quantile|percentile|statistics?|distribution|box ?plot|median|quartile|decile|histogram|regression)\b", re.I)
...
URL_SIGNAL_RE  = re.compile(r"\bfractile\.ai\b", re.I)
...
def fetch_url_text(url):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent":
              "Mozilla/5.0 (compatible; FractileMentionBot/1.0; +https://fractile.ai)"})
    ...
    for tid in ids:
        ...
        try:
            d = _get_json(f"https://api.fxtwitter.com/status/{tid}",
                          {"User-Agent": "FractileMentionBot/1.0 (+https://fractile.ai)"}, retries=2)
        except Exception:
            continue
...
def structural(t):
    ...
    # Fractile is also a statistical term; reject obvious non-company usage.
...
def judge(t, content=None):
    ...
    system = (
        "You classify tweets/links for a media-monitoring bot at Fractile, a London AI inference-hardware "
        "company. Fractile builds processors, systems and software that physically interleave memory and "
        "compute to improve latency, throughput and cost for frontier-model inference. Its founder and CEO "
        "is Walter Goodwin; leadership includes Pete Hughes and Chris Smith. Decide whether this is ABOUT "
        "Fractile the company, its technology, people, jobs, funding, or news. Beware false positives: "
        "'fractile' is a statistical quantile term, and Fractile Labs is an unrelated carbon-materials company."
    )
...
def review_anchor_ts(state):
    ...
    ts = slack_api(SLACK_BOT_TOKEN, SLACK_REVIEW_CHANNEL,
                   f"Fractile mention — review queue ({date})",
...
def main():
    ...
    for t, content, kind in judge_queue:
        ...
        if not rel:
            continue                                   # we looked — it isn't Fractile -> drop

#!/usr/bin/env python3
"""
Fractile LinkedIn mention bot
===========================
LinkedIn sibling of monitor.py: finds new public LinkedIn posts mentioning
Fractile (the AI inference-hardware company) and posts them to the same Slack channel.
...
Filtering   : reuses monitor.py's tiered classifier — strong signals auto-
              accept, clear statistical false positives drop cheaply, ambiguous
...
def structural_li(item):
    ...
    if a.get("type") == "company" and (
            ...
            or (a.get("name") or "").strip().lower() == "fractile"):
        return "own_post", []
    txt = text_of(item)
    if not txt:
...
def judge_li(item):
    ...
    user = f"Author: {who}\nLinkedIn post:\n{text_of(item)[:3500]}"
    system = (
        "You classify LinkedIn posts for a media-monitoring bot at Fractile, a London AI inference-hardware "
        ...
        "is Walter Goodwin; leadership includes Pete Hughes and Chris Smith. Decide whether this post is ABOUT "
...
def review_anchor_ts_li(state):
    ...
    ts = mon.slack_api(mon.SLACK_BOT_TOKEN, mon.SLACK_REVIEW_CHANNEL,
                       f"Fractile LinkedIn mentions — review queue ({date})",

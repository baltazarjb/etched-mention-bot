...
MatX mention bot
==================
Finds every new X/Twitter mention of MatX (the AI hardware company) and posts it to Slack.
...
Routing:
  confident MatX              -> MAIN channel (clean feed of real mentions)
  confident NOT MatX          -> dropped (we looked, it isn't us)
...
STRONG_TEXT_RE = re.compile(r"matx\.com", re.I)
ETCHED_WORD_RE = re.compile(r"(?<![a-z])matx(?![a-z])", re.I)
SOHU_WORD_RE   = re.compile(r"(?<![a-z])matx(?:\s+one)?(?![a-z])", re.I)
MATX_ONE_RE    = re.compile(r"(?<![a-z])matx\s+one(?![a-z])", re.I)
# “mATX” is a PC motherboard form factor, and makes up the overwhelming
# majority of unrelated X matches.  It is never a MatX-company signal.
MATX_PC_RE = re.compile(
    r"\b(micro[ -]?atx|motherboard|mainboard|placa[- ]m[aã]e|"
    r"form factor|pc case|cabinet|a[345]\d\d|b[345]\d\d|am[345]|ddr[345])\b", re.I)
...
URL_SIGNAL_RE  = re.compile(r"\bmatx\.com\b", re.I)
...
def fetch_url_text(url):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent":
              "Mozilla/5.0 (compatible; MatXMentionBot/1.0; +https://matx.com)"})
    ...
    for tid in ids:
        ...
        try:
            d = _get_json(f"https://api.fxtwitter.com/status/{tid}",
                          {"User-Agent": "MatXMentionBot/1.0 (+https://matx.com)"}, retries=2)
        except Exception:
            continue
...
def relevant_excerpt(text, limit=3500):
    ...
    entirely. Return the lede plus a window around every brand-keyword hit."""
...
def structural(t):
    ...
    # “mATX” motherboard posts are a high-volume, unrelated source of noise.
...
def judge(t, content=None):
    ...
    system = (
        "You classify tweets/links for a media-monitoring bot at MatX, a Mountain View AI hardware company. "
        "MatX designs chips and systems for large language models. Its MatX One chip targets high throughput, "
        "low latency, SRAM-resident weights, HBM-backed KV cache, training, RL and inference. Its founders are "
        "Reiner Pope and Mike Gunter. Decide whether this is ABOUT MatX the company, MatX One, its people, jobs, "
        "funding, research, or news. Beware false positives: 'MatX' can refer to unrelated organizations or a "
        "generic shorthand; the Israeli manufacturing accelerator MatX (matx-il.com) is NOT this company."
    )
...
def review_anchor_ts(state):
    ...
    ts = slack_api(SLACK_BOT_TOKEN, SLACK_REVIEW_CHANNEL,
                   f"MatX mention — review queue ({date})",
...
def main():
    if not TWAPI_KEY or not (SLACK_MAIN or (SLACK_BOT_TOKEN and SLACK_MAIN_CHANNEL)):
        print("FATAL: set TWAPI_KEY and either SLACK_WEBHOOK_URL or SLACK_BOT_TOKEN + SLACK_MAIN_CHANNEL", file=sys.stderr); sys.exit(1)
    dry = "--dry" in sys.argv
    ...
    for t, content, kind in judge_queue:
        ...
        if not rel:
            continue                                   # we looked — it isn't MatX -> drop
    ...
    if dry:
        ...
        for t, reasons, v in sorted(to_main, key=keyf):
            try:
                fb, bl = build_msg(t, reasons, v, "main"); send_main(fb, bl)

#!/usr/bin/env python3
"""
MatX LinkedIn mention bot
===========================
LinkedIn sibling of monitor.py: finds new public LinkedIn posts mentioning
MatX (the AI hardware company) and posts them to the same Slack channel.
...
Filtering   : reuses monitor.py's tiered classifier — strong signals auto-
              accept, and ambiguous references go to the conservative judge
...
QUERIES = ["MatX", "MatX One", "Reiner Pope", "Mike Gunter"]
...
def paused_alert(state, credit, dry=False):
    ...
    if dry:
        ...
        mon.send_main("LinkedIn monitoring paused — Apify credit exhausted",
...
def structural_li(item):
    ...
    if a.get("type") == "company" and (
            ...
            or (a.get("name") or "").strip().lower() == "matx"):
        return "own_post", []
    txt = text_of(item)
    if not txt:
...
def judge_li(item):
    ...
    user = f"Author: {who}\nLinkedIn post:\n{text_of(item)[:3500]}"
    system = (
        "You classify LinkedIn posts for a media-monitoring bot at MatX, a Mountain View AI hardware company. "
        ...
        "Reiner Pope and Mike Gunter. Decide whether this post is ABOUT MatX the company, MatX One, its people, "
        "jobs, funding, research, or news. Beware false positives: 'MatX' can refer to unrelated organizations; "
        "the Israeli manufacturing accelerator MatX (matx-il.com) is NOT this company."
    )
...
def review_anchor_ts_li(state):
    ...
    ts = mon.slack_api(mon.SLACK_BOT_TOKEN, mon.SLACK_REVIEW_CHANNEL,
                       f"MatX LinkedIn mentions — review queue ({date})",
...
# ------------------------------------------------------------------ main
def main():
    if not APIFY_TOKEN or not (mon.SLACK_MAIN or (mon.SLACK_BOT_TOKEN and mon.SLACK_MAIN_CHANNEL)):
        print("FATAL: set APIFY_TOKEN and a Slack main destination", file=sys.stderr); sys.exit(1)
    dry = "--dry" in sys.argv
    ...
    if dry:
        ...
        for it, reasons, v in sorted(to_main, key=keyf):
            try:
                fb, bl = build_msg_li(it, reasons, v, "main", warn); mon.send_main(fb, bl)
